...
# NODES
##
class FlowState_QuickEdit:
    CATEGORY = 'FlowState Creator Suite/Editing'
    DESCRIPTION = 'A 2-stage node that makes edits to an image using Qwen Image Edit, and then refines the image with Flux for quality enhancement.'
    FUNCTION = 'execute'
    RETURN_TYPES = TYPE_QUICK_EDIT_OUT
    ...
    def execute(
        ...
        latent_batch_in = FlowState_SimpleLatent().execute(
            resolution, orientation, latent_type, custom_width, custom_height,
            custom_batch_size, self.working_vae, input_img
        )[0]
